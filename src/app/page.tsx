...
const PROVIDERS = [
  ...
  { id: "nvidia", name: "NVIDIA", color: "#76b900", emoji: "🚀" },
]
...
export default function Vikarma() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("vikarma_api_keys")
    if (saved) {
      try {
  ...
  const saveApiKey = (providerId: string, key: string) => {
    ...
    setApiKeys(newKeys)
    localStorage.setItem("vikarma_api_keys", JSON.stringify(newKeys))
    setSaveStatus(`✅ ${PROVIDERS.find(p => p.id === providerId)?.name} key saved!`)
  ...
  const send = async () => {
    ...
    try {
      ...
      const url = apiKey
        ? `http://127.0.0.1:8765/chat?api_key=${encodeURIComponent(apiKey)}`
        : "http://127.0.0.1:8765/chat"

      const res = await fetch(url, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ message: text, provider: provider.id })
      }).then(r => r.json()).catch(() => ({
        response: `${provider.emoji} [${provider.name}] Sacred temples awakening... Start backend: npm run server\n\n"${text}"\n\nOm Namah Shivaya 🔱`
      }))

      setMessages(m => [...m, {
        role: "assistant",
        content: res.response || res.error || "🔱",
  ...
  const S: any = {
    sidebar: { width: 64, display: "flex", flexDirection: "column", alignItems: "center", padding: "16px 0", gap: 8, background: "rgba(3,0,26,0.95)", borderRight: "1px solid rgba(192,132,252,0.1)", flexShrink: 0 },
    header: { display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 24px", borderBottom: "1px solid rgba(192,132,252,0.1)", background: "rgba(3,0,26,0.6)", flexShrink: 0 }
  }
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            {PROVIDERS.map(p => (
              <button
                ...
                style={{
                  ...
                  fontSize: 12,
                  cursor: "pointer",
                  background: provider.id === p.id ? `${p.color}20` : "transparent",
                  border: `1px solid ${provider.id === p.id ? p.color : "rgba(255,255,255,0.1)"}`,
                  color: provider.id === p.id ? p.color : "rgba(255,255,255,0.3)"
                }}
              >
                {p.emoji} {p.name}
              </button>
            ...
            <div style={{ padding: 16, borderTop: "1px solid rgba(192,132,252,0.1)", flexShrink: 0 }}>
              <div style={{ display: "flex", gap: 12, alignItems: "flex-end" }}>
                <textarea
                  ...
                  placeholder={`Message ${provider.name}... (Enter to send)`}
                  rows={2}
                  style={{ flex: 1, borderRadius: 12, padding: "10px 16px", fontSize: 14, resize: "none", outline: "none", background: "rgba(3,0,26,0.8)", border: "1px solid rgba(192,132,252,0.2)", color: "rgba(240,240,255,0.9)", fontFamily: "inherit" }}
                />
                <button
                  onClick={send}
                  disabled={loading || !input.trim()}
                  style={{ padding: "10px 20px", borderRadius: 12, fontSize: 13, fontWeight: 700, cursor: "pointer", background: input.trim() ? `${provider.color}20` : "rgba(255,255,255,0.05)", border: `1px solid ${input.trim() ? provider.color : "rgba(255,255,255,0.1)"}`, color: input.trim() ? provider.color : "rgba(255,255,255,0.3)" }}
                >
                  Send 🔱
                </button>
              </div>
        ...
        {tab === "temples" && (
          <div style={{ flex: 1, overflowY: "auto", padding: 24 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 32 }}>
              {ASHTA.map((t, i) => (
                <div key={i} style={{ borderRadius: 12, padding: 16, cursor: "pointer", background: "rgba(3,0,26,0.8)", border: `1px solid ${t.color}30` }}>
                  <div style={{ fontSize: 28, marginBottom: 8 }}>{t.icon}</div>
                  <div style={{ fontSize: 13, fontWeight: 700, color: t.color, fontFamily: "monospace" }}>{t.name}</div>
                  <div style={{ fontSize: 11, opacity: 0.5, marginTop: 4 }}>{t.domain}</div>
                </div>
              ))}
            </div>
        ...
        {tab === "settings" && (
          <div style={{ flex: 1, overflowY: "auto", padding: 24, maxWidth: 700 }}>
            <div style={{ fontSize: 20, fontWeight: 700, color: "#ffd700", marginBottom: 8 }}>⚙️ API Configuration</div>
            <div style={{ fontSize: 12, opacity: 0.6, marginBottom: 24 }}>Configure your AI provider API keys. Keys are stored locally in your browser.</div>

            {saveStatus && (
              <div style={{ padding: "10px 16px", borderRadius: 8, background: "rgba(0,255,136,0.1)", border: "1px solid #00ff88", color: "#00ff88", fontSize: 13, marginBottom: 16 }}>
                {saveStatus}
              </div>
            )}

            {PROVIDERS.map(p => (
              <div key={p.id} style={{ marginBottom: 20, padding: 16, borderRadius: 12, background: "rgba(3,0,26,0.6)", border: `1px solid ${p.color}30` }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                  <span style={{ fontSize: 18 }}>{p.emoji}</span>
                  <span style={{ fontSize: 14, fontWeight: 700, color: p.color }}>{p.name}</span>
                  {apiKeys[p.id] && (
                    <span style={{ fontSize: 11, color: "#00ff88", marginLeft: "auto" }}>✅ Configured</span>
                  )}
                </div>
                <div style={{ display: "flex", gap: 8 }}>
                  <input
                    ...
                    defaultValue={apiKeys[p.id] || ""}
                    onBlur={(e) => saveApiKey(p.id, e.target.value)}
                    style={{ flex: 1, borderRadius: 8, padding: "10px 14px", fontSize: 13, outline: "none", background: "rgba(0,0,0,0.4)", border: `1px solid ${p.color}40`, color: "white", fontFamily: "monospace" }}
                  />
                  {apiKeys[p.id] && (
                    <button
                      onClick={() => saveApiKey(p.id, "")}
                      style={{ padding: "10px 16px", borderRadius: 8, fontSize: 12, cursor: "pointer", background: "rgba(255,71,87,0.2)", border: "1px solid #ff4757", color: "#ff4757" }}
                    >
                      Clear
                    </button>
                  )}
                </div>
                <div style={{ fontSize: 10, opacity: 0.4, marginTop: 8, color: p.color }}>
                  Env: {API_KEY_NAMES[p.id]}
                </div>
              </div>
            ))}

            <div style={{ marginTop: 32, padding: 20, borderRadius: 12, background: "rgba(255,215,0,0.05)", border: "1px solid rgba(255,215,0,0.2)", fontSize: 12, opacity: 0.7, lineHeight: 1.8 }}>
              🔱 <strong>Vikarma</strong> — Unlicense — Free for All Humanity<br />
              Built with love for Shiva Mahadeva 🕉️<br />
              Not for us. For all. Har Har Mahadev 🔱
            </div>
          </div>
        )}
      </div>
